...
function preload() {
    ...
    icon = loadImage('assets/img/emoji.webp');
    // font = loadFont("assets/fonts/ReliefSingleLineCAD-Regular.ttf");
...
function drawMovingDots() {
    clear();
    let speed = 0.5; // slow speed, now works!

    characters.forEach((character, index) => {
        const characterPoints = character.points;
        if (characterPoints.length < 2) return; // skip if not enough points 
        let iconNum = Math.floor(characterPoints.length / 4);
        let iconSize = 40;

        assignColortoCharacter(character);

        for (let i = 0; i < iconNum; i++) {
            let progress = (tracker + i * 10) % characterPoints.length;

            let indexA = Math.floor(progress);
            let indexB = (indexA + 1) % characterPoints.length;
            let t = progress - indexA;

            let ptA = characterPoints[indexA];
            let ptB = characterPoints[indexB];

            let x = lerp(ptA.x, ptB.x, t);
            let y = lerp(ptA.y, ptB.y, t);

            image(icon, x, y, iconSize, iconSize);
        }
    });

    tracker += speed; // adjust this to control overall animation speed
}
